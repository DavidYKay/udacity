...
def recursive_tour(accum, nodes):
    if not nodes:
        accum.append((accum[-1][1], accum[0][0]))
        return accum
    else:
        if accum:
            lastPair = accum[-1]
            accum.append((lastPair[1], nodes.pop()))
        else:
            accum.append((nodes.pop(), nodes.pop()))
        return recursive_tour(accum, nodes)

def create_tour(nodes):
    # create a simple loop
    return recursive_tour([], list(nodes))
...
def is_eulerian_tour(nodes, tour):
    ...
    if len(connected) == len(nodes):
        ...
        print "len(connected): " + str(len(connected))
